...
const updateProfile = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      updateData.imagePath = `${process.env.BASE_URL.replace(
        /\/?$/,
        "/"
...
const getAllDoctors = async (req, res) => {
  try {
    const doctors = await Doctor.find({ status: "Accepted" })
      .populate("userId", "name email contactNumber gender");

    // ✅ filter out broken references
    const validDoctors = doctors.filter(doc => doc.userId);

    res.status(200).json({
      success: true,
      doctors: validDoctors.map((doc) => ({
        _id: doc._id,
